const getProductFromForm = (slug = -1) => {
	...
	formValues['productName'] = formElement.querySelector(
		'input#p_product_name',
	).value;
	formValues['oldPrice'] = parseInt(
		formElement.querySelector('input#p_old_price').value,
	);
	formValues['discountPercentage'] = parseInt(
		formElement.querySelector('input#p_discount_percentage').value,
	);
	...
	formValues['manufacturer'] = formElement.querySelector(
		'input#p_manufacturer',
	).value;
	formValues['stockQuantity'] = parseInt(
		formElement.querySelector('input#p_stock_quantity').value,
	);
	formValues['material'] = formElement.querySelector('input#p_material').value;
	formValues['operatingSystem'] = formElement.querySelector(
		'input#p_operating_system',
	).value;
	formValues['cpuType'] =
		formElement.querySelector('select#p_cpu_type').options[
			formElement.querySelector('select#p_cpu_type').selectedIndex
		].value;
	formValues['available'] = parseInt(
		formElement.querySelector('select#p_available').options[
			formElement.querySelector('select#p_available').selectedIndex
		].value,
	);
	formValues['gpuOnboard'] = parseInt(
		formElement.querySelector('select#p_gpu_onboard').options[
			formElement.querySelector('select#p_gpu_onboard').selectedIndex
		].value,
	);
